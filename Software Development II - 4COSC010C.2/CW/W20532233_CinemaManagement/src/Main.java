...
public class W20532233_CinemaManagement {
    // Define arrays
    ...
    static void Menu() {
        ...
        int option = user_input.nextInt();  //get input from user

        if (option <= 8 && option >= 1) {   //checking if the user input is in the correct range
            switch (option) {
                case 8:
                    System.out.println("exit your programme");
                    System.exit(0); // If user enters 8 then Exit the program

                case 1:
                    buy_ticket();  //if user input is 1, go to buy ticket method
                    skip();
                    break;

                case 2:
                    cancel_ticket();    //if user input is 2, go to cancel ticket method
                    skip();
                    break;

                case 3:
                    print_seating_area();     //if user input is 3, go to print_seating_area method
                    skip();
                    break;

                case 4:
                    find_first_available();    //if user input is 4, go to find_first_available method
                    skip();
                    break;

                case 5:
                    print_tickets_info();     //if user input is 5, go to  print_tickets_info method
                    skip();
                    break;

                case 6:
                    search_tickets();     //if user input is 6, go to search_ticket method
                    skip();
                    break;

                case 7:
                    sort_tickets();     //if user input is 7, go to sort_tickets method
                    skip();
                    break;
    ...
    static void details_buy(String row_number) {
        int column_number;  //initialize variables
        int seatPrice;

        if (row_number.equalsIgnoreCase("A") || row_number.equalsIgnoreCase("D")) {     //check if row letter is A or B
            while (true) {
        ...
        } else if (row_number.equalsIgnoreCase("B") || row_number.equalsIgnoreCase("C")) {    //check if row letter is b or c
            while (true) {
        ...
        int row_index = row_number.charAt(0) - 'A';
    ...
    //calling print_seating_area method
